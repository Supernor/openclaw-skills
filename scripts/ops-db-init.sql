-- ops.db schema v1
-- OpenClaw operational database — shared between Claude Code (host) and agents (container)
...
-- Config changes: backfilled from config-audit.jsonl
...
-- Incidents: created/resolved by incident-manager.sh
...
-- Tasks: inbox/outbox for Claude Code <-> agent communication
CREATE TABLE IF NOT EXISTS tasks (
    ...
    errors TEXT,
    outcome TEXT,
    result TEXT,      -- JSON blob from Claude Code response
    meta TEXT         -- JSON blob
);
...
-- Notifications: model health notifications (replaces JSONL)
CREATE TABLE IF NOT EXISTS notifications (
    ...
    delivered INTEGER DEFAULT 0,  -- 0=pending, 1=sent to Discord
    meta TEXT  -- JSON blob
);
...
CREATE TABLE IF NOT EXISTS agent_results (
    ...
    for_agent TEXT NOT NULL DEFAULT 'any',
    type TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_bus_for ON agent_results(for_agent) WHERE consumed = 0;
CREATE INDEX IF NOT EXISTS idx_bus_task ON agent_results(task_id) WHERE task_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_bus_type ON agent_results(type);
