...
# --- 4. repo-health (verify all 3 repos are fresh) ---
echo "[4/4] repo-health..."
...
if [ -f "$LAST_RUN" ]; then
    if [ "$FAILURES" -eq 0 ]; then
        echo "$TIMESTAMP | backup-suite | PASS | env=${ENV_STATUS} skills=${SKILLS_STATUS} ws=${WS_STATUS} repos=${HEALTH_STATUS}" >> "$LAST_RUN"
    else
        echo "$TIMESTAMP | backup-suite | FAIL | ${FAILURES} failures: env=${ENV_STATUS} skills=${SKILLS_STATUS} ws=${WS_STATUS}" >> "$LAST_RUN"
    fi
fi
